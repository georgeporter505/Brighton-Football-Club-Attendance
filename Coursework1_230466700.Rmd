...
subtitle: "Brighton&Hove Albion Match Attendance" 
...
We will start with creating our dataframe, I have used fbref <https://fbref.com/en/squads/d07537b9/Brighton-and-Hove-Albion-Stats#all_matchlogs>
to compile Brighton's match data and attendance numbers, as well as <https://www.transfermarkt.co.uk/premier-league/besucherzahlen/pokalwettbewerb/GB1>
transfer markt to find information on each stadium.
...
```
Now I will add the stadium names, averages and capacities into the data frame by creating stadium lookup vectors and information columns
```{r}
...
Brighton_Away = merge(
    ...
    all.x = TRUE,
    sort = FALSE
) # merged Brighton away and stadium info using the column for opponents
...
```
Now we calculate a demand or attractiveness score which compares Brighton's
...
I had issues creating this data frame all at once so I had to construct it
...
This is putting our dataframe into prophet
```{r}
library(prophet)
m = prophet(df_prophet)  
```
Here I am creating a sequence of the months we're plotting over, in order to
label our axis.
```{r}
...
Now we will make future predictions as well as historical, so we can analyse
fit of our model to make comments about accuracy.
```{r}
...
```
combine the forecast and historic data to plot a graph.
```{r}
...
grid() # looks nice and helps visualise the data.
```
